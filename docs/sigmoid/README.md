# Sigmoid（S 型激活函数）模块设计说明

## 模块目标

本模块用于演示 CUDA 中逐元素 **sigmoid（S 型激活函数）** 的实现：

```text
y[i] = 1 / (1 + exp(-x[i])),  i = 0 .. N-1
```

同时展示同一个运算的不同优化写法：

1. 标量版本：一个线程处理一个元素；
2. 向量化版本（`float4` / `half2` / 128-bit 打包）：一个线程一次处理多个连续元素；
3. FP16 版本：用更小精度的数据类型，成倍降低访存字节数。

本模块与 elementwise 模块是**一对姊妹示例**：6 个 kernel 版本一一对应，唯一的
区别是把 `c[i] = a[i] + b[i]` 换成 `y[i] = sigmoid(x[i])`。elementwise 里
讲过的线程编号、向量化访存、FP16 打包等知识本模块不再重复，重点放在
**sigmoid 特有的数值稳定性与精度问题**上。

最终目标不是追求极致性能，而是让学习者直观理解 CUDA 编程模型与访存优化思想。

## 本次改动范围

本模块参照 elementwise 模块的做法，统一补充教学注释，改动原则是
**只加注释、不改 kernel 逻辑**，具体包含：

1. 先更新本文档（`docs/sigmoid/README.md`）；
2. 给 `kernels/sigmoid/sigmoid.cu` 补充分节标题、调用链、grid/block 速查、
   exp 溢出保护与 FP16 精度说明，以及逐行行内注释；
3. 给 `kernels/sigmoid/sigmoid.py` 补充模块 docstring 与行内注释，并新增
   打印 PyTorch CUDA 扩展实际构建目录、以及打印当前 GPU 设备名的代码
   （对齐 `elementwise.py`）。

`scripts/README.md` 中已存在 sigmoid 模块的运行命令，本次不再改动。

向量化版本（`f32x4` / `f16x2` / `f16x8` / `f16x8_pack`）缺少尾部（tail）分支的
问题在本文档中已识别并说明，但**本次不修复**，留待后续单独处理。

## 涉及文件

| 文件 | 作用 |
| --- | --- |
| `kernels/sigmoid/sigmoid.cu` | CUDA kernel 定义 + PyTorch C++ 绑定（代码中已添加详细注释） |
| `kernels/sigmoid/sigmoid.py` | 用 `torch.utils.cpp_extension.load` 编译并执行基准测试 |
| `kernels/sigmoid/README.md` | 模块使用说明与测试输出 |

## 注释导览

对照 elementwise 模块，`sigmoid.cu` 中的注释按下面的顺序组织：

| 位置 | 内容 |
| --- | --- |
| 文件头 | 模块公式、6 个版本概览、与 elementwise 的对应关系、学习重点 |
| 宏定义段 | `reinterpret_cast` 向量访存宏的用法与对齐要求 |
| `MAX_EXP_*` / `MIN_EXP_*` | exp 溢出保护边界的数值来历，以及为什么要先 clamp 再 exp |
| `sigmoid_f32_kernel` | 调用链、grid/block 速查、数值保护与 `expf` 选型说明、逐行注释 |
| `sigmoid_f32x4_kernel` | `float4` 向量化访存、逐分量展开计算、缺少 tail 分支的风险 |
| `sigmoid_f16_kernel` | FP16 常数转换、`hexp` / `__hmin` / `__hmax` 选型 |
| `sigmoid_f16x2_kernel` | `half2` 双分量计算 |
| `sigmoid_f16x8_kernel` | unpack 写法：4 个 `half2` 完成 8 个元素 |
| `sigmoid_f16x8_pack_kernel` | pack 写法：局部数组 + 128 位整体访存 |
| `TORCH_BINDING_SIGMOID` | host 端宏展开细节与调用链 |
| `PYBIND11_MODULE` | 导出给 Python 的函数名 |

`TORCH_BINDING_SIGMOID` 宏内部的注释使用 `/* ... */` 块注释而非 `//`：
宏定义每行以 `\` 续行，而 `//` 会把该行末尾的续行符一起注释掉，宏定义会当场断开。
`/* ... */` 块注释在预处理阶段被替换为空格，不会影响续行。

## CUDA 核心概念回顾

### 1. 线程组织

和 elementwise 一样，kernel 启动后每个线程通过以下内置变量确定自己的位置：

| 变量 | 含义 |
| --- | --- |
| `threadIdx.x` | 当前线程在 block 内的编号 |
| `blockIdx.x` | 当前 block 在 grid 内的编号 |
| `blockDim.x` | 每个 block 的线程数 |

全局线程编号为：

```c
int idx = blockIdx.x * blockDim.x + threadIdx.x;
```

### grid / block 知识点速查

- 一次 kernel 启动 `kernel<<<grid, block>>>(...)` 只创建一个 grid；
  `grid` 是“本次启动要创建的所有 block 的集合”。
- 尖括号内两个配置的含义（标准写法为 `<<<Dg, Db>>>`）：
  - `grid`：本 grid 一共有几个 block，进入 kernel 后对应 `gridDim.x`；
  - `block`：每个 block 有几个线程，进入 kernel 后对应 `blockDim.x`。
  - `grid` / `block` 只是本文件里 `dim3` 局部变量的名字，不是 CUDA 关键词。
- kernel 内部固定内置变量：
  - `gridDim.x`：总 block 数；`blockIdx.x`：当前 block 的编号（从 0 开始）；
  - `blockDim.x`：每 block 线程数；`threadIdx.x`：线程在 block 内编号（从 0 开始）。
- 全局线程编号 `idx = blockIdx.x * blockDim.x + threadIdx.x`：
  只需要知道“当前 block 前面有几个 block”，不需要知道 `gridDim.x`。
- 本模块的约定是**每个 block 处理 256 个元素**：标量版 block = 256 线程，
  向量化版 block = 256 / n_elements（每线程处理 n_elements 个元素），
  grid 都取 `ceil(N / 256)`。
- 例：`<<<4, 256>>>`、`N = 1000`：
  - `gridDim.x = 4`、`blockDim.x = 256`，总线程数 1024；
  - block 0/1/2/3 分别负责下标 0~255、256~511、512~767、768~1023；
  - 下标 1000~1023 的 24 个线程被 `if (idx < N)` 拦截，不参与计算。

### 2. Sigmoid 的数学性质

```text
sigmoid(x) = 1 / (1 + exp(-x))
```

- **值域**：`(0, 1)`，两端饱和。`x` 很大时结果趋近 1，`x` 很小时结果趋近 0；
- **单调递增**，且关于点 `(0, 0.5)` 中心对称：`sigmoid(-x) = 1 - sigmoid(x)`；
- **导数**：`sigmoid'(x) = sigmoid(x) * (1 - sigmoid(x))`，最大值 0.25 出现在
  `x = 0`，这也是它在神经网络里比阶跃函数更适合做激活函数的原因之一。

函数曲线（ASCII 示意图，`x ∈ [-6, 6]`，`+` 标出对称中心 `(0, 0.5)`）：

```text
1.0 ┤                     **********
    ┤                  ***
    ┤                **
0.5 ┤               +
    ┤             **
    ┤          ***
0.0 ┤**********
     └──────────────────────────────→ x
     -6      -3     0      3       6
```

采样值：`sigmoid(-6) ≈ 0.0025`、`sigmoid(-3) ≈ 0.0474`、`sigmoid(0) = 0.5`、
`sigmoid(3) ≈ 0.9526`、`sigmoid(6) ≈ 0.9975`。

- **两端的含义**：`|x|` 超过约 8 之后输出在 FP32 里就已经是 `1.0` 或 `0.0`，
  这正是下文 kernel 先做 clamp 再算 exp 的依据——饱和后的钳位不改变结果，
  却能避免 `exp` 溢出；
- **作为激活函数**：它给网络引入非线性（否则多层线性变换叠加仍是线性变换），
  缺点是两端导数趋近 0、深网络容易梯度消失（现代隐藏层多用 ReLU），
  但二分类输出层、LSTM 门控等场景仍然常用。

### 3. 为什么必须先 clamp 再 exp

直接计算 `1.0f / (1.0f + expf(-x))` 在极端输入下会出问题：

- `x` 非常小（例如 `-1000`）时 `-x = 1000`，`expf(1000)` 溢出成 `inf`，
  `1 + inf = inf`，`1 / inf = 0`——结果虽然“碰巧”正确，但中间出现了 inf；
- 更麻烦的是 `x` 含有 `inf` / `nan` 时，整条链路都会传播 `inf` / `nan`；
- FP16 的指数函数更容易溢出：half 最大值只有 65504，`hexp` 的参数
  超过约 11.09 就溢出。

本模块的做法是**先把输入夹到 exp 不会溢出的区间**，再做指数运算：

```c
float v = x[idx];
v = fminf(fmaxf(v, MIN_EXP_F32), MAX_EXP_F32);
y[idx] = 1.0f / (1.0f + expf(-v));
```

sigmoid 在两端本来就是饱和的，超出这个区间后 FP32 能表示的结果已经分别是
`1.0f` 或 `0.0f`（FP16 同理），因此钳位不会改变最终可表示的结果，
只是把“溢出后的偶然正确”换成了“全程有限且确定”。

### 4. 溢出边界的数值来历

| 宏 | 取值 | 含义 |
| --- | --- | --- |
| `MAX_EXP_F32` | `88.3762626647949f`（≈ `ln(2^127.5)`） | 略小于 `ln(FLT_MAX)` ≈ 88.723，是常用的保守上界 |
| `MIN_EXP_F32` | `-88.3762626647949f` | 对称的负向下界 |
| `MAX_EXP_F16` | `__float2half(11.089866488461016f)` ≈ `ln(65504)` | `65504` 是 half 的最大规格数 |
| `MIN_EXP_F16` | `__float2half(-9.704060527839234f)` ≈ `ln(2^-14)` | `2^-14` 是 half 的最小规格正数 |

注意 FP16 的两个边界是**不对称**的：上界 11.09 很大、下界只有 -9.70，
因为 half 能表示的正数范围本身就偏向 0 一侧（最小规格正数 6.1e-5，
最大规格数 65504）。

### 5. FP16 精度提醒

FP32 尾数 23 位，最小刻度约 `2^-23 ≈ 0.00000012`（约 7 位有效数字）；
FP16 尾数只有 10 位，最小刻度为
`2^-10` = 2 的 10 次方分之一 = `1/1024` ≈ `0.00098`
（约 3~4 位有效数字，保守按约 3 位）。

FP32 转 half 会就近取整产生量化误差，例如同一数值 FP32 显示 `1.223737`、
FP16 显示 `1.22460938`，属于预期精度损失。sigmoid 的输入输出都落在
`(0, 1)` 附近，量化误差在测试输出里体现为小数点后第 4 位左右的差异。

## Kernel 设计

所有 kernel 都遵守一个约定：**把输入输出都当作长度为 `N` 的一维数组处理**。
二维矩阵 `(S, K)` 在启动端被展平为 `N = S * K`。

### 1. `sigmoid_f32_kernel`（FP32 标量版）

```c
int idx = blockIdx.x * blockDim.x + threadIdx.x;
if (idx < N) {
  float v = x[idx];
  v = fminf(fmaxf(v, MIN_EXP_F32), MAX_EXP_F32);
  y[idx] = 1.0f / (1.0f + expf(-v));
}
```

- 一个线程只算一个元素；
- `if (idx < N)` 处理 `N` 不能被 block 大小整除时"多启动"的线程；
- `fminf` / `fmaxf` / `expf` 是 float 版本的数学函数，不能用 double 版
  `fmin` / `fmax` / `exp`（后者会走 double 运算路径，明显更慢）；
- 正确性最直观，是后面所有版本的基准。

### 2. `sigmoid_f32x4_kernel`（FP32 向量化版）

每个线程连续处理 **4 个 float（16 字节）**：

```c
int idx = (blockIdx.x * blockDim.x + threadIdx.x) * 4;
float4 reg_x = FLOAT4(x[idx]);
float4 reg_y;
```

- 将一段连续内存 reinterpret 成 `float4`，用一次加载/存储完成 16 字节搬运；
- 与 elementwise 不同，sigmoid 是**单输入**算子：只需读 `x` 一份数据；
- 向量化省的是访存指令条数，`expf` 这类计算仍要按 `x / y / z / w`
  四个分量逐条展开；
- 因为每个线程有 4 个独立元素，寄存器压力比标量版更大。

### 3. `sigmoid_f16_kernel`（FP16 标量版）

```c
const half f = __float2half(1.0f);
half v = x[idx];
v = __hmin(__hmax(v, MIN_EXP_F16), MAX_EXP_F16);
y[idx] = f / (f + hexp(-v));
```

- 数据类型为 `half`（2 字节），字节数减半，是后续 FP16 向量化的基础；
- `1.0` 这个常数先用 `__float2half` 转成 `half` 存进变量 `f` 复用，
  避免每个元素都做一次类型转换；
- 绝对值保护用 `__hmin` / `__hmax`（直接操作 16 位 half），
  指数用 half 版本 `hexp`，不必先提升到 float；
- 注意 half 没有 float 那样的隐式类型提升，混用类型容易触发额外的转换指令。

### 4. `sigmoid_f16x2_kernel`（FP16 每线程 2 元素）

- 一次用 `half2`（2 个 half = 4 字节）完成两个元素的搬运；
- `half2` 只有 `x`、`y` 两个分量，两个分量各自独立计算 sigmoid；
- 由于没有 `hexp2` 这样的成对指数函数，指数仍要按分量分别调用 `hexp`。

### 5. `sigmoid_f16x8_kernel`（FP16 每线程 8 元素，unpack 写法）

```c
int idx = (blockIdx.x * blockDim.x + threadIdx.x) * 8;
half2 reg_x_0 = HALF2(x[idx + 0]);
half2 reg_x_1 = HALF2(x[idx + 2]);
half2 reg_x_2 = HALF2(x[idx + 4]);
half2 reg_x_3 = HALF2(x[idx + 6]);
```

- 每个线程处理 8 个 half，拆成 4 个 `half2`（`idx+0/2/4/6`）分别处理；
- 相比 f16x2，进一步摊薄索引计算与循环开销；
- 结果是 4 个 `half2`，分别写回 `y[idx+0/2/4/6]`。

### 6. `sigmoid_f16x8_pack_kernel`（128-bit 打包版）

```c
half pack_x[8], pack_y[8]; // 8x16 bits=128 bits.
LDST128BITS(pack_x[0]) = LDST128BITS(x[idx]); // load 128 bits
...
LDST128BITS(y[idx]) = LDST128BITS(pack_y[0]);
```

- 把 8 个 half（正好 16 字节 = 128 位）装入局部数组；
- 借助 reinterpret 成 `float4` 的 `LDST128BITS` 宏，让一次访存指令读写完整的
  128 位，比 unpack 版本的多次 `half2` 访存更省指令；
- 局部数组在 PTX 中通常对应 `.local` 空间（可寻址），
  但配合 `#pragma unroll` 与编译期常数下标，编译器有机会把它优化到寄存器里，
  从而避免真正访问 local memory；
- 计算部分用 `#pragma unroll` 展开的循环完成，每轮处理 1 个 half。

## 边界约束（已识别，本次不修复）

elementwise 的向量化版本用 `(idx + 3) < N` 判断整段元素是否齐全，
并配了尾部标量回退分支；本模块的向量化版本**两个都没有**，
越界判断只检查了段的起始下标：

| kernel | 现有判断 | `N` 不是向量宽度的整数倍时会发生什么 |
| --- | --- | --- |
| `sigmoid_f32x4_kernel` | `(idx + 0) < N` | 最后一个 block 的部分线程会把 `idx+1..idx+3` 写到 `y` 合法范围之外（越界写 1~3 个 float） |
| `sigmoid_f16x2_kernel` | `(idx + 0) < N` | 同上，越界写 1 个 half |
| `sigmoid_f16x8_kernel` | `(idx + 0/2/4/6) < N` | 同上，越界写 1 个 half |
| `sigmoid_f16x8_pack_kernel` | `(idx + 7) < N` | 方向相反：末尾不足 8 个的元素被整块**丢弃**（漏算），但不会越界 |

本模块基准测试的 `S`/`K` 都取 1024 的倍数（元素数是 256 的倍数），
启动的线程恰好整段对齐，因此不会暴露该问题。

本次只加注释、不改逻辑，所以该风险仅在本文档和源码注释中写明。
如需在生产中使用，应参照 elementwise 补充尾部处理：

```c
if ((idx + 3) < N) {
  // 4 个元素都有效，走 float4 向量化分支
} else if (idx < N) {
  // 尾部不足 4 个元素，退回逐元素处理
}
```

## 启动配置（host 端）

启动端把每个 block 负责的元素数固定为 **256**：

```c
dim3 block(256 / (n_elements));   // 每个线程负责 n_elements 个元素
dim3 grid((N + 256 - 1) / 256);   // 向上取整
```

各版本的参数对应关系：

| 版本 | `n_elements` | block 线程数 | 每 block 处理元素数 |
| --- | ---: | ---: | ---: |
| `sigmoid_f32` | 1 | 256 | 256 |
| `sigmoid_f32x4` | 4 | 64 | 256 |
| `sigmoid_f16` | 1 | 256 | 256 |
| `sigmoid_f16x2` | 2 | 128 | 256 |
| `sigmoid_f16x8` | 8 | 32 | 256 |
| `sigmoid_f16x8_pack` | 8 | 32 | 256 |

二维输入 `(S, K)` 且每行元素数除以 `n_elements` 不超过 1024 时，
采用"一行一个 block"的启动方式：

```c
dim3 block(K / (n_elements));
dim3 grid(S);
```

当某行需要的线程数超过硬件上限（通常 1024）时，回退到展平启动。

## PyTorch 绑定

`sigmoid.cu` 通过 `TORCH_BINDING_SIGMOID` 宏批量生成 6 个 host 函数，
再经 `PYBIND11_MODULE` 暴露为 Python 可调用模块，主要工作：

1. 用 `CHECK_TORCH_TENSOR_DTYPE` 检查输入输出张量数据类型；
2. 根据维度/形状计算 `grid` 与 `block`；
3. 用 `reinterpret_cast<element_type *>(x.data_ptr())` 拿到裸指针；
4. 以 `kernel<<<grid, block>>>` 启动内核。

### 调用链（以 `sigmoid_f32` 为例）

```text
sigmoid.py
  └─ lib.sigmoid_f32(x, y)                    # Python 调用（pybind11）
      └─ sigmoid_f32(torch::Tensor x, y)      # C++ host 包装函数（宏生成）
          ├─ CHECK_TORCH_TENSOR_DTYPE(x/y)    # 校验 dtype 必须是 float32
          ├─ 计算 dim3 block / dim3 grid
          └─ sigmoid_f32_kernel<<<grid, block>>>(...)  # CUDA 启动语法
              └─ cudaLaunchKernel(...)        # nvcc 生成，交给 CUDA runtime
                  └─ GPU 各线程并行执行 kernel 函数体
```

要点：

- Python 调用的 `sigmoid_f32` 运行在 CPU 上，负责类型检查、计算 `grid` / `block`；
- `sigmoid_f32_kernel` 运行在 GPU 上，必须用 `<<<>>>` 启动，不能普通函数调用；
- 启动后 GPU 的每个线程都会执行一遍 kernel 函数体，靠 `threadIdx` / `blockIdx`
  区分自己负责的下标 `idx`。

## 测试脚本

`sigmoid.py` 流程：

1. 用 `torch.utils.cpp_extension.load` 现场编译 `sigmoid.cu`；
2. 对 `S ∈ {1024, 2048, 4096}`、`K ∈ {1024, 2048, 4096}` 组合生成随机张量；
3. `run_benchmark` 先做 warmup，再运行 1000 次取平均；
4. 依次对比 6 个自定义 kernel 与 PyTorch 官方 `torch.sigmoid` 的正确性和耗时。

脚本在 `load(...)` 完成后会打印 PyTorch CUDA 扩展的实际构建目录，
便于确认 `sigmoid_lib.so` 的存放位置；随后用
`print(torch.cuda.get_device_name())` 打印当前 GPU 设备名，
便于对照不同显卡上的耗时数据。

打印对照结果前，`run_benchmark` 会通过
`out.flatten().detach().cpu().numpy().tolist()[:2]` 把 GPU 上的结果张量
拉平、断开可能的梯度追踪（本脚本已关闭 autograd，属于保险写法）、拷回 CPU、
转成 Python 列表，并只取前 2 个元素用于人工核对。

## 性能观察与结论

从 `kernels/sigmoid/README.md` 的测试输出可总结：

- FP16 各版本一般快于对应 FP32 版本，因为访存字节减半；
- 同数据类型下，向量化版本普遍快于标量版本
  （例如 `S=4096, K=4096` 时标量 `f32` 约 0.186 ms、`f32x4` 约 0.186 ms
  基本持平，而 `f16` 约 0.050 ms、`f16x8_pack` 约 0.021 ms）；
- `f16x8_pack` 在多数规模下最快，验证"更宽的访存 + 更小的数据类型"的组合收益；
- 小矩阵或极端规模下耗时差异可能不明显，甚至出现波动，
  因为 kernel 时间已接近启动开销量级；
- sigmoid 每元素要做一次指数运算，比 elementwise 加法"计算更重"，
  所以纯访存优化的收益比 elementwise 更容易被计算时间掩盖。

## 后续可以尝试的优化方向

- 补上向量化版本的尾部（tail）分支，使任意 `N` 都安全；
- 用 `tanh` 或 `exp2` 近似 exp，或改用 `__frcp` 等快速数学函数；
- 利用 `sigmoid(-x) = 1 - sigmoid(x)` 把输入折到单侧，减少一半溢出判断；
- 在 FP16 输入下用 FP32 累加计算再转回，提升精度；
- 引入 grid-stride loop，使固定大小的 grid 也能处理任意大 `N`；
- 对比不同 `block` 大小、不同向量宽度对带宽的实测影响。
